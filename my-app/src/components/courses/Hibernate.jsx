...
const Syllabus = () => {
  ...
  useEffect(() => {
    if (profile?.courses?.['hibernate']?.completed) {
      const numCompleted = Math.floor((profile.courses['hibernate'].completed / 100) * 5);
  ...
  const completeChapter = async (id) => {
    if (!completedChapters.includes(id)) {
      ...
      await updateProgress('hibernate', newProgress);
    }
  };

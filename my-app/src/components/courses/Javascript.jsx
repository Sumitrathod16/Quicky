...
const Syllabus = () => {
  ...
  useEffect(() => {
    if (profile?.courses?.['javascript']?.completed) {
      const numCompleted = Math.floor((profile.courses['javascript'].completed / 100) * 5);
  ...
  const completeChapter = async (id) => {
    if (!completedChapters.includes(id)) {
      ...
      await updateProgress('javascript', newProgress);
    }
  };
